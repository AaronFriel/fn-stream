...
type TokenType = 'eof' | 'string' | 'boolean' | 'null' | 'numeric' | 'punctuator';
...
export class StreamingParser<T extends JsonValue = any> {
  ...
  private stateStack: Array<JsonArray | JsonObject>;
      ...
      case 'stringEscapeUnicode': {
        ...
        if (isHexDigit(this.nextChar)) {
          ...
          if (this.unicodeEscapeBuffer.length === 4) {
            this.writeStringBuffer(
              String.fromCodePoint(Number.parseInt(this.unicodeEscapeBuffer, 16)),
            );
            this.lexState = 'string';
          }
  ...
  private newToken<T extends TokenType>(type: T, value?: TokenValue[T], partial = false): Token {
  ...
  private partialArray(): PartialArray {
    ...
    return value as PartialArray;
  }

  private push() {
    let value: JsonValue | undefined;
    let stateValue: PartialPrimitiveValue | PartialObject | PartialArray | undefined;
    ...
    if (this.outputStack.length === 0) {
      if (this.stream && typeof value !== 'object') {
        throw new Error(`Unexpected input, expected object or array. Found ${this.token.type}`);
      }
      this.root = value;
      this.stateRoot = stateValue;
    } else if (Array.isArray(current)) {
      const outputArray = current as JsonArray;
      const stackArray = this.stateStack[this.stateStack.length - 1] as PartialArray;
      if (this.pushed) {
      ...
      path = this.key;
      Object.defineProperty(this.outputStack[this.outputStack.length - 1], this.key!, {
        value,
        writable: true,
        enumerable: true,
        configurable: true,
      });
      Object.defineProperty(this.stateStack[this.stateStack.length - 1], this.key!, {
        value: stateValue,
        writable: true,
        enumerable: true,
        configurable: true,
    ...
    if (typeof value !== 'object') {
      let currentPath = path !== undefined ? [...this.pathStack, path] : [...this.pathStack];
  ...
  private invalidChar(c: string | EOF) {
    ...
    return this.syntaxError(
      `JSON5: invalid character '${this.formatChar(c)}' at ${this.line}:${this.column}`,
    );
  }
  ...
  private syntaxError(message: string | undefined) {
    const error = new SyntaxError(message) as SyntaxError & {
      lineNumber: number;
      columnNumber: number;
      state: string;
    };
    error.lineNumber = this.line;
    error.columnNumber = this.column;
    error.state = JSON.stringify(this);
    return error;
  }
